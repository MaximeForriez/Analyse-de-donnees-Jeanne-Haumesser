...
def main():
	...
	pop2007 = df_monde[pop2007_col] if pop2007_col else [float('nan')] * len(etats)
	pop2025 = df_monde[pop2025_col] if pop2025_col else [float('nan')] * len(etats)
	dens2007 = df_monde[dens2007_col] if dens2007_col else [float('nan')] * len(etats)
	dens2025 = df_monde[dens2025_col] if dens2025_col else [float('nan')] * len(etats)
	...
	print(f"{len(etats_communs_2007)} pays présents dans les deux classements (pop2007 & densité 2007).")
